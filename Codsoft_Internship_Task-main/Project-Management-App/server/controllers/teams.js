...
export const updateTeam = async (req, res, next) => {
  try {
    ...
    return next(
      createError(
        403,
        "You can update only if you are a member of this Teams!",
      ),
    );
  } catch (err) {
    next(err);
...
export const inviteTeamMember = async (req, res, next) => {
  ...
  const link = `${appUrl}/team/invite/${req.app.locals.CODE}?teamid=${req.params.id}&userid=${req.body.id}&access=${req.body.access}&role=${req.body.role}`;
