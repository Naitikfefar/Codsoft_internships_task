...
const hasMailCredentials =
  ...
  process.env.EMAIL_USERNAME !== "your_email@example.com" &&
  process.env.EMAIL_PASSWORD !== "your_email_app_password";

const sendOtpMail = (mailOptions) =>
...
export const generateOTP = async (req, res, next) => {
  ...
  const mailOptions =
    reason === "FORGOTPASSWORD" ? resetPasswordOtp : verifyOtp;

  try {
    const sent = await sendOtpMail(mailOptions);
    return res.status(200).send({
      message: sent ? "OTP sent" : "OTP generated locally",
      otp:
        process.env.NODE_ENV === "production" ? undefined : req.app.locals.OTP,
    });
  } catch (err) {
